...
func (m *OrderInfoResponse) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetStatus()); l < 3 || l > 140 {
	...
	if !_OrderInfoResponse_Status_Pattern.MatchString(m.GetStatus()) {
		err := OrderInfoResponseValidationError{
			field:  "Status",
			reason: "value does not match regex pattern \"^(new|cancel|payed|awaiting_payment)$\"",
		}
		if !all {
			return err
		}
...
var _OrderInfoResponse_Status_Pattern = regexp.MustCompile("^(new|cancel|payed|awaiting_payment)$")
