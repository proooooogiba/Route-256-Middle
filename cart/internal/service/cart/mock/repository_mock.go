...
func (mmDeleteItemsByUserID *mRepositoryMockDeleteItemsByUserID) ExpectUserIDParam2(userID int64) *mRepositoryMockDeleteItemsByUserID {
	if mmDeleteItemsByUserID.mock.funcDeleteItemsByUserID != nil {
		mmDeleteItemsByUserID.mock.t.Fatalf("RepositoryMock.DeleteItemsByUserID mock is already set by Set")
	}
...
// DeleteItemsByUserID implements cart.Repository
func (mmDeleteItemsByUserID *RepositoryMock) DeleteItemsByUserID(ctx context.Context, userID int64) (err error) {
	...
	if mmDeleteItemsByUserID.DeleteItemsByUserIDMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmDeleteItemsByUserID.t.Errorf("RepositoryMock.DeleteItemsByUserID got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.userID != nil && !minimock.Equal(*mm_want_ptrs.userID, mm_got.userID) {
				mmDeleteItemsByUserID.t.Errorf("RepositoryMock.DeleteItemsByUserID got unexpected parameter userID, want: %#v, got: %#v%s\n", *mm_want_ptrs.userID, mm_got.userID, minimock.Diff(*mm_want_ptrs.userID, mm_got.userID))
			}
